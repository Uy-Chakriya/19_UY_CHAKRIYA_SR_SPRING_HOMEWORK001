...
public final List<Ticket> TICKET_LIST = new ArrayList<>();
    ...
    @PostMapping("/bulk")
    public ApiResponse<List<Ticket>> addTickets(@RequestBody List<List<Ticket>> tickets){
    ...
    public ResponseEntity<?> getTicketById(@PathVariable("ticket-id") Integer id){
    ...
    @PutMapping("/bulk")
    public ApiResponse<List<Ticket>> updateTickets(@RequestBody List<List<Ticket>> tickets){
        for(List<Ticket> ticketList : tickets){
            for(Ticket ticket : ticketList){
                for(Ticket nTicket : TICKET_LIST){
                    if(nTicket.getTicketId().equals(ticket.getTicketId())){
                        nTicket.setPaymentStatus(ticket.getPaymentStatus());
                    }
                }
            }
